...
from keras.layers import Bidirectional, LSTM, Dropout, Dense, Activation, LeakyReLU, Input, Conv1D, MaxPooling1D, RepeatVector, Flatten, SimpleRNN
...
#    plt.show()
    ...
    return predicted_crash, true_labels, len_test
...
# load data
dataset = pd.read_csv('site1_1min.csv', index_col=[0], header=[0,1])
# dataset1 = pd.read_csv('site2_5min.csv', index_col=[0], header=[0,1])
...
for i in range(5):
    ...
    for j in range(1):
            ...
            for k,length in enumerate(len_series):
                    ...
                    model = Sequential()
                    model.add(SimpleRNN(100, activation=LeakyReLU(0.5), input_shape=(length, num_features)))
                    ...
                    # Compile the model
                    model.compile(optimizer=SGD(lr=0.00001, momentum=0.8), loss='binary_crossentropy',metrics=['binary_accuracy'])
                    model.summary()

                    predicted_labels,true_labels,len_test = result_model(model, x_train_res, y_train_res, x_test, y_test, length, num_epochs=70, batch_size=32, num_features=num_features)
                    print("number of crashes: {}\n".format(sum(true_labels == 1)))

                    for l,f in enumerate(flex):
                            tp, fn, tn, fp = confusion_matrix(predicted_labels, true_labels, len_test, f)
