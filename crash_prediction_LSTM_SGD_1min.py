...
dataset1 = pd.read_csv('site2_3min.csv', index_col=[0], header=[0,1])
...
len_series = np.array([6,7,8,9,10])
...
for i in range(5):
    ...
    for j in range(1):
            ...
            for k,length in enumerate(len_series):
                    ...
                    # build the model
                    model = Sequential()
                    model.add(LSTM(100, return_sequences=False, activation=LeakyReLU(0.5), dropout=0.4, input_shape=(length, num_features)))
                    ...
                    # Compile the model
                    model.compile(optimizer=SGD(learning_rate=0.00001, momentum=0.8), loss='binary_crossentropy', metrics=['binary_accuracy'])
                    model.summary()
